locals {
    ...
    additional_ports = [
        for indx, additional_port in var.additional_ports: {
            ...
            service_port = lookup(additional_port, "ingress_port", lookup(additional_port, "service_port", null))
            path = local.rewrite_path_enabled ? "${lookup(additional_port, "path", "")}/(.*)" : lookup(additional_port, "path", "")
        }
        if lookup(additional_port, "name", null) != null && lookup(additional_port, "ingress_port", lookup(additional_port, "service_port", null)) != null && lookup(additional_port, "path", null) != null
    ]
